...
The repository includes GitHub Actions workflows for automated daily feeds. The CI configuration is controlled by the `config.ci.yaml` file.

### Changing Summary Language for CI

To change the language of summaries generated by the CI workflow:

1. **Edit the CI configuration file** (`config.ci.yaml`):
   ```yaml
   topic: "quantum computing"
   language: "ja"  # Change from "en" to "ja" for Japanese summaries
   max_results: 20
   top_n: 5
   # ... rest of configuration
   ```

2. **Available language options**:
   - `"en"` - English summaries (default)
   - `"ja"` - Japanese summaries

3. **Example configurations**:
   - For **English** CI summaries (current default):
     ```yaml
     language: "en"
     ```
   - For **Japanese** CI summaries:
     ```yaml
     language: "ja"
     ```

The CI workflow (`.github/workflows/daily-feed.yml`) automatically uses the `config.ci.yaml` file, so any changes to the language setting will be applied to the automated daily feed generation.

**Note**: After changing the language setting, commit and push the changes to the repository. The next scheduled run or manual workflow trigger will use the new language setting.
